...
class module_class extends AWS_MODEL
{
	...
	//新增加随机热门
	public function top_hot_users($uid = 0, $avatra = 0, $limit = 5, $times = 7)
	{
		$avar='';
		if($avatra<>0){$avar='avatar_file <>"" AND ';}
		if ($users_list = $this->fetch_all('users', ''.$avar.' last_active > ' . (time() - (60 * 60 * 24 * $times)), 'answer_count DESC', ($limit * 4)))
		{
			foreach($users_list as $key => $val)
			{
				if (!$val['url_token'])
