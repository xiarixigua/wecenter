...
class ajax extends AWS_CONTROLLER
{
    ...
    public function publish_article_action()
    {
        ...
        else
        {
			...
			if(count($img_array)>0){
				...
				{
					$value = trim($value);
                    if(strtolower(substr($value,0,4))=='http'){//此处判断有待后期验证，主要考虑到很多url中没有.jpg等这样的标识
            ...
            $article_id = $this->model('publish')->publish_article($_POST['title'], $body, $this->user_id, $_POST['topics'], $_POST['category_id'], $_POST['attach_access_key'], $this->user_info['permission']['create_topic'],$_POST['abstract']);
    ...
    public function modify_article_action()
    {
        ...
        $this->model('article')->update_article($article_info['id'], $_POST['title'], $body, $_POST['topics'], $_POST['category_id'], $this->user_info['permission']['create_topic'],$_POST['abstract']);
