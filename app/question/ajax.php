...
class ajax extends AWS_CONTROLLER
{
	...
	public function save_answer_fox_action()
	{
		if (trim($_POST['message']) == '')
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('请输入回复内容')));
		}
		if (!$question_info = $this->model('question')->get_question_info_by_id($_POST['foxid']))
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('问题不存在')));
		}
		
		if(FOX_ReplyId == ''){
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('参数有误无法继续')));
		}else{
			if(FOX_ReplyId){
				$foruid=explode('|',FOX_ReplyId);
				$num = rand(0, count($foruid)-1);
				$findexid=$foruid[$num];
			}else{
				$findexid=$this->user_id;
			}			
		}

		
		$this->model('publish')->publish_answer($question_info['question_id'], $_POST['message'], $findexid);
	...
	public function save_answer_action()
	{
		...
		else
		{
			...
			$answer_id = $this->model('publish')->publish_answer($question_info['question_id'], $body, $this->user_id, $_POST['anonymous'], $_POST['attach_access_key'], $_POST['auto_focus']);
	...
	public function update_answer_action()
	{
		...
		$this->model('answer')->update_answer($_GET['answer_id'], $answer_info['question_id'], $body, $_POST['attach_access_key']);
