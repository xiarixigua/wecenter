...
class ajax extends AWS_CONTROLLER
{
	...
	//新增
	public function article_read_action()
	{
		if (! $article_info = $this->model('article')->get_article_info_by_id(intval($_POST['article_id'])))
		{
			H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('文章不存在')));
		}else{
			$article_info = $this->model('article')->get_article_info_by_id(intval($_POST['article_id']));
			...
			if ($article_info['has_attach'])
			{
                ...
                foreach ($article_info['attachs'] AS $attach) { 
					if ($attach['is_image'] AND (!$article_info['attachs_ids'] OR !in_array($attach['id'], $article_info['attachs_ids']))) {
                        $article_info['img'].='<p><a href="'.$attach['attachment'].'" target="_blank" data-fancybox-group="thumb" rel="lightbox"><img src="'.$attach['attachment'].'" class="img-polaroid" alt="'.$attach['file_name'].'" /></a></p>';
                    }
				}
			}
			$article_info['message'] = FORMAT::parse_attachs(FORMAT::parse_bbcode($article_info['message']));
			H::ajax_json_output(AWS_APP::RSM(array('ajax_html' => $article_info['message'].$article_info['img']), 1, null));
		}
	}
	//结束
